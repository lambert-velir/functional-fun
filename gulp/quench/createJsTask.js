...
module.exports = function jsTask(taskName, userConfig) {
  ...
  const getNpmPackages = createNpmPackagesGetter(
    jsConfig.files,
    librariesTaskName,
  );
  ...
  gulp.task(librariesTaskName, function() {
    ...
    const b = browserify(
      Object.assign({}, jsConfig.browserify, {
        require: npmPackages,
      }),
    );
  ...
  quench.maybeWatch(
    taskName,
    R.unnest([jsConfig.watch, findup.sync("package.json")]),
  );
...
function bundleJs(browserifyOptions, npmPackages) {
  return through2.obj(function(file, enc, callback) {
    ...
    const b = browserify(browserifyOptions || {}) // pass options
      ...
      .transform(babelify, {
        // run it through babel, for es6 transpiling
        presets: ["env", "react"],
